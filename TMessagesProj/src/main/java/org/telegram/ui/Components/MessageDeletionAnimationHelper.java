...
public class MessageDeletionAnimationHelper extends TextureView {
    private AnimationThread thread;

    public MessageDeletionAnimationHelper(@NonNull Context context) {
        super(context);
        init();
    }

    public MessageDeletionAnimationHelper(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MessageDeletionAnimationHelper(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private static class AnimationThread extends Thread {
        ...
        private static final int maxPointSize = AndroidUtilities.dp2(1.5f) * 2;
        private final int visibleSize = Math.max(1, AndroidUtilities.dp2(0.5f) * 2);
        ...
        private static final float UP_ACCELERATION = 280 * TIME_SCALE;
        private static final float EASE_IN_DURATION = 1.2f / TIME_SCALE;
        private static final float MIN_LIFETIME = 1.2f / TIME_SCALE;
        private static final float MAX_LIFETIME = 2.5f / TIME_SCALE;
        ...
        private void init() {
            ...
            GLES31.glShaderSource(vertexShader, RLottieDrawable.readRes(null, R.raw.deletion_shader2) + "\n// " + Math.random());
            ...
            GLES31.glShaderSource(fragmentShader, RLottieDrawable.readRes(null, R.raw.deletion_shader) + "\n// " + Math.random());
