...
JNIEXPORT jint Java_org_telegram_solo_MediaController_startRecord(JNIEnv *env, jclass class, jstring path, jint sampleRate) {
...
JNIEXPORT jint Java_org_telegram_solo_MediaController_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame((uint8_t *) frameBytes, (uint32_t) len);
}

JNIEXPORT void Java_org_telegram_solo_MediaController_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}

JNIEXPORT jint Java_org_telegram_solo_MediaController_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT jbyteArray Java_org_telegram_solo_MediaController_getWaveform2(JNIEnv *env, jclass class, jshortArray array, jint length) {
...
JNIEXPORT jbyteArray Java_org_telegram_solo_MediaController_getWaveform(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    jbyteArray result = 0;
